...
function add_p_class(){
            $.ajax({
                type: 'POST',
                url: 'route/controller.php?action=add_bg',
...
function gotolist(){
    ...
    else
    {
        $('#pClassTable').DataTable({
            "bProcessing": true,
            "sAjaxSource": "route/controller.php?action=bglist",
            "aoColumns": [
...
function loadEvent(_id){
  ...
  var dataAry = {
    ...
    $.post('route/controller.php?action=loadbg', dataAry, function(data, textStatus, xhr) {
            postData=data.data;
...
//                alert(data.errmsg);
              ...
              break;
              case "success":  
				$("#ed_bg_name").val(postData[0].name);
				$("#ed_bg_code").val(postData[0].code);
...
function saveEvent(){
	  $.ajax({
			type: 'POST',
			url: 'route/controller.php?action=editbg',
...
function deleteEvent(_id){
  if(confirm("確定要刪除嗎?不能後悔喔!"))
  {
    ...
    $.post('route/controller.php?action=deletebg', dataAry, function(data, textStatus, xhr) {
            switch(data.message){
              case "failed":
                swal({
                    title: "場景刪除失敗！",
