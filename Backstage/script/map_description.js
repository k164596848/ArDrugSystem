...
function add_p_class(){
            ...
            $.ajax({
                type: 'POST',
                url: 'route/controller.php?action=add_map_des',
...
function gotolist(){
    ...
    else
    {
        $('#pClassTable').DataTable({
            "bProcessing": true,
            "sAjaxSource": "route/controller.php?action=load_branch_des&ser="+id,
            "aoColumns": [
                { "mData": "num"},
				{ "mData": "branch_content"},
                { "mData": "EDITOR"},
...
//                alert(data.errmsg);
              ...
              break;
              case "success":  
				var content = '<li class="active"><a href ="mind_map?param1=branch1">概念圖管理 </a></li> <li><a href = \'mind_map?param1=branch2&ser='+postData[0].first_itemno.split('-')[0]+'\'>'+postData[0].first_itemno.split('-')[1] +' </a></li> <li><a href = \'mind_map?param1=branch3&ser='+postData[0].second_itemno.split('-')[0] +'\'>'+postData[0].second_itemno.split('-')[1]+' </a></li> <li><a href = \'mind_map?param1=branch4&ser='+postData[0].third_itemno.split('-')[0] +'\'>'+postData[0].third_itemno.split('-')[1]+' </a></li>';
